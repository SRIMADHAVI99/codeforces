...
The grasshopper is very fond of positive integers, so for each integer i starting with  the following holds: exactly i minutes after the start he makes a jump with a distance of exactly i. So, in the first minutes he jumps by 1, then by 2, and so on.
...
Each of the following t lines contains two integers x0 (−1014≤x0≤10^14) and n (0≤n≤10^14) — the coordinate of the grasshopper's initial position and the number of jumps.
